...
plot_dr_bifurcation
    Two-panel figure zoomed to the Detroit River AOC only.

plot_dr_map (helper)
    Render only the Detroit River AOC shapefile on a matplotlib Axes.
"""
...
# ---------------------------------------------------------------------------
# Detroit River (DR) — zoomed-in variants
# ---------------------------------------------------------------------------
...
def plot_dr_map(
    ...
    *,
    ...
    """Draw **only** the Detroit River AOC onto *ax*.
    ...
    maps_dir : str or Path
        Root directory containing the shapefile sub-folders.
    annotate : bool
        Whether to add a water-body name label.
    annotation_fontsize : int
        Font size for annotation labels.
    ...
    maps_dir = Path(maps_dir)
    shp = maps_dir / "detroit_river_aoc_shapefile" / "AOC_MI_Detroit_2021.shp"

    if shp.exists():
        gdf = gpd.read_file(shp).to_crs(epsg=4326)
        gdf.plot(ax=ax, color="lightblue", edgecolor="steelblue",
                 alpha=0.55, linewidth=0.6)

    # Tight bounds around the Detroit River AOC + site scatter
    ax.set_ylim(42.00, 42.40)
    ax.set_xlim(-83.25, -82.85)

    ax.grid(linestyle="--", alpha=0.35)

    if annotate:
        kw = dict(fontsize=annotation_fontsize, color="gray", style="italic")
        ax.text(-83.12, 42.01, "Detroit River", **kw)

    return ax


def plot_dr_bifurcation(
    scores: pd.Series,
    lat: pd.Series,
    lon: pd.Series,
    waterbody: pd.Series,
    ...
    *,
    ...
    """Two-panel figure zoomed to the Detroit River AOC.

    Same layout as :func:`plot_corridor_bifurcation` but the left panel
    shows only the Detroit River shapefile for visual clarity.

    Parameters
    ----------
    (Same as ``plot_corridor_bifurcation``.)
    """
    # ── resolve styles ────────────────────────────────────────────────────
    wb_styles = waterbody_styles or _WATERBODY_STYLE
    c_below  = below_color  or _BIFURCATION_COLORS["below"]
    ...
    # ── LEFT: Detroit River map ───────────────────────────────────────────
    plot_dr_map(ax_map, maps_dir, annotate=True,
                annotation_fontsize=label_fontsize - 2)
