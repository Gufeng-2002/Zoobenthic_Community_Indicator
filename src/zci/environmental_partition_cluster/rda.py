        ...
        **kwargs
            ...
            if site_groups is not None:
                    ...
                    for group in unique_groups:
                        ...
                        ax.scatter(group_sites.iloc[:, 0], group_sites.iloc[:, 1],
                                ...
                                label=str(group), zorder=3)
                        
                        # Add site labels with matching colors
                        for site in group_sites.index:
                            x = site_scores.loc[site, :].iloc[0] - 0.05
                            y = site_scores.loc[site, :].iloc[1] + 0.05
        ...
        if show_species:
            ...
            for species in species_to_show:
                if species in species_scores.index:
                    x, y = species_scores.loc[species, :]
                    ax.text(x * 2, y * 2, species, fontsize=8, color='red', alpha=0.7,
        ...
        if show_env:
            for var in biplot_scores.index:
                ...
                ax.arrow(0, 0, x, y, head_width=0.05, head_length=0.08,
                        fc='blue', ec='blue', alpha=0.7, linewidth=1.5, zorder=4)
                ...
                ax.text(x * 1.2 + 0.2, y * 1.2 - 0.05, var + p_text, fontsize=8, color='blue',
        ...
        # Labels and styling
        ax.set_xlabel(f'{axis_names[0]} ({expl_var.iloc[0]:.1f}%)', fontsize=12, fontweight='bold')
        ax.set_ylabel(f'{axis_names[1]} ({expl_var.iloc[1]:.1f}%)', fontsize=12, fontweight='bold')
